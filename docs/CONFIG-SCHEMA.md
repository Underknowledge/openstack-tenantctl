...
**Use case**: A project operates in domain `"acme-eu"` for OpenStack purposes, but the IDP mapping should reference `"Default"` (or no domain at all) instead.

**Examples**:
```yaml
# Project uses acme-eu for OpenStack, but IDP mapping omits domain
domain: "acme-eu"
federation:
...
# Project uses acme-eu for OpenStack, but IDP mapping uses a different domain
domain: "acme-eu"
federation:
...
domain: "acme-eu"
federation:
  # domain absent or "" → inherits "acme-eu" from project
  role_assignments:
    - idp_group: member
      roles: [member]
```
